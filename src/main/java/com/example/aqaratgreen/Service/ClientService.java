...
public class ClientService {
    ...
    public List<Unit> getUnitOfferType(String offerType) {
        ...
        if(listOfferType.isEmpty()) {
            throw new ApiException("**** لايوجد قائمة وحدات المطلوبه ***");
        }
    ...
    public double calculateAverageContractValue(Integer clientId) {
        ...
        if (clientContracts.isEmpty()) {
            throw new ApiException("**** لايوجد عقود برقم العميل ****");
        }
    ...
    // جمع أو تحصيل إيجار من عميل معين لوحدة سكنية معينة
    public double collectRent(Integer clientId, Integer unitId) {
        Client client = clientRepository.findClientById(clientId);
        Unit unit = unitRepository.findUnitById(unitId);

        if (unit == null) {
            throw new ApiException("****** لايوجد وحدة سكنية بهذا الرقم ******");
        }
        if (client == null) {
            throw new ApiException("***** لايوجد عميل بهذا الرقم ******");
        }
