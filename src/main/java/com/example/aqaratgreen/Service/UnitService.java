...
public class UnitService {
    ...
    public void addUnit(Unit unit) {
        if (unit.getOfferType().equalsIgnoreCase("rent")) {
            ...
            unitRepository.save(unit);
              ...
              throw new ApiException(" نوع العرض غير معروف، يرجى تحديد 'rent' أو 'buy' ");
        }
    }
    ...
    public void deleteUnit(Integer id) {
        ...
        if (u ==null) {
            throw new  ApiException("No Unit found with id "+id);
        }
...
// --------------------------------- End Point ----------------------------------
    ...
    public double calculateMonthlyRentCost(Integer unitId, int additionalRooms) {
        ...
        if (unit==null) {
            throw new  ApiException(" الوحدة السكنية غير موجوده بالنظام "+unitId+" *****");
        }
        else if (!unit.getOfferType().equalsIgnoreCase("rent")) {
            throw new ApiException(" ****** الوحدة ليست للإيجار ****** ");
        }
    ...
    public String calculateSustainabil(Integer unitId) {
        Unit unit = unitRepository.findUnitById(unitId);
        if (unit == null) {
            throw new ApiException("***** لايوجد بالنظام وحدة سكنية برقم  " + unitId+"*******");
        }

        Criteria criteria = criteriaRepository.findCriteriaById(unit.getCriteriaId());
        if (criteria == null) {
            throw new ApiException("******لا يوجد خصائص برقم  " + unitId+" ******");
        }
    ...
    public String PaymentAndRemainAmountForUnit(Integer clientId, Integer unitId) {
        ...
        if (client == null) {
            throw new ApiException(" العميل برقم ****" + clientId + " .****ليس في النظام");
        }

        if (unit == null) {
            throw new ApiException(" الوحدة السكنية برقم**** " + unitId + ".**** ليست في النظام");
        }
        ...
        if (remainingAmount < 0) {
            throw new ApiException("**** Error: Total amount paid exceeds total amount due for unit " + unit.getName());
...
public Map<String,List<Unit>> expectedProfitUnitPerYear() {
    ...
    if (allUnits == null || allUnits.isEmpty()) {
        throw new ApiException("**** لايوجد وحدات سكنية متوفرة في النظام ****");
    } else {
        ...
        if (expectedList.isEmpty()) {
            throw new ApiException("لايوجد توقعات ربحية للوحدات السكنية الموجودة");
        }
    }
