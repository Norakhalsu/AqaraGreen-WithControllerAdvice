...
public class UnitController {
    ...
    public ResponseEntity addUnit(@Valid @RequestBody Unit unit) {
        ...
        return ResponseEntity.status(201).body(new ApiResponse(" تم اضافة الوحدة السكنية بنجاح "));
    }


    @PutMapping("/update/{id}")// update Unit by id
    public ResponseEntity updateUnit(@PathVariable Integer id ,@Valid@RequestBody Unit unit ) {
        unitService.updateUnit(id,unit);
        return ResponseEntity.status(201).body(new ApiResponse("تم تحديث الوحدة السكنية بنجاح "));
    }
    ...
    public ResponseEntity deleteUnit(@PathVariable Integer id){
        ...
        return ResponseEntity.status(201).body(new ApiResponse("تم حذف الوحدة السكنية بناح "));
    }
